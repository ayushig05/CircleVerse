...
exports.verifyAccount = catchAsync(async(req, res, next) => {
    const { otp } = req.body;
    if (!otp) {
        return next(new AppError("OTP is required for verification", 404));
    }
    const user = req.user;
    if (user.otp !== otp) {
...
exports.resendOtp = catchAsync(async(req, res, next) => {
    ...
    try {
        await sendEmail({
            email: user.email,
            subject: "Resend OTP for email verfication",
            html: htmlTemplate,
        });
        res.status(200).json({
            status: "success",
        ...
        user.otpExpires = undefined;
        await user.save({ validateBeforeSave: false });
        return next(new AppError("There is an error in sending email. Try again later!", 500));
    }
});
...
exports.forgetPassword = catchAsync(async(req, res, next) => {
    ...
    const htmlTemplate = loadTemplate("otpTemplate.hbs", {
        ...
        username: user.username,
        otp,
        message: "Your reset one-time password (OTP) is: ",
    });
    try {
        await sendEmail({
            email: user.email,
